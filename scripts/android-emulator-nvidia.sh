...
if [[ "$AVD_NAME" == Scolive_Dev_AOSP_API33* ]] || [[ "$AVD_NAME" == Scolive_GooglePlay_API33* ]]; then
  GPU_MODE="swiftshader_indirect"
fi

# Le Google Play AVD charge le snapshot "startup_ready" si disponible (démarrage rapide,
# évite la recompilation dex2oat au premier boot qui bloque l'app ~30 min).
# L'AOSP démarre toujours sans snapshot (état propre garanti).
if [[ "$AVD_NAME" == Scolive_GooglePlay_API33* ]]; then
