/**
 * Fiche santé élève — vue responsable santé / admin / manager, réutilisée en
 * lecture seule (sans FAB ni édition des soins) pour l'enseignant référent
 * d'une classe accédant à la fiche d'un de ses élèves.
...
import { MultiActionFab } from "../navigation/MultiActionFab";
...
import { HEALTH_SCHOOL_TOUR_TARGETS } from "./health-school-tour.config";
...
export function AdminHealthStudentScreen() {
  const { t } = useTranslation();
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const { schoolSlug, user } = useAuthStore();
  const isReadOnlyReferentView = user?.activeRole === "TEACHER";
              ...
              <InfiniteScrollList
                ...
                renderItem={({ item }) => (
                  <HistoryCard
                    ...
                    item={item}
                    onEdit={
                      item.kind === "CARE_EVENT" && !isReadOnlyReferentView
