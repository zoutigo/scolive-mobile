...
- ✅ Toujours tester `user.activeRole === "TEACHER"`, `isResourcePlatformAdmin(activeRole)`, etc.
- ❌ Ne jamais faire `user.memberships.some(m => m.role === "TEACHER")` ou `user.platformRoles.length > 0` pour décider d'un accès ou d'un affichage : ça réactive un droit qui n'appartient pas au rôle actuellement actif.
